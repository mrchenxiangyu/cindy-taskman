...
`
    ...
    function apply(ctx) {
      ...
      const connect = () => {
        if (!connected) {
          connected = (async () => {
            ...
            // 命名空间挂载可能有极短异步延迟：轮询片刻再判定，避免误报失败
            let ns = ctx.get('remote.cindy')
            for (let i = 0; i < 8 && !ns; i++) {
              await new Promise((res) => setTimeout(res, 60))
              ns = ctx.get('remote.cindy')
            }
            if (!ns) throw new Error('Cindy 远程命名空间未挂载')
            lastTransport = 'remote'
      ...
      const call = async (m, a) => {
        const rpc = await withTimeout(connect(), 4000, 'Cindy 连接超时，请重试')
      ...
      const sleep = (ms) => new Promise((res) => {
        if (timer) { timer.timeout(res, ms); return }
        setTimeout(res, ms)
      })
      // refresh：支持自动重试退避 —— 应用/连接启动期间打开面板不再干等超时，
      // 而是短间隔重试，DSH 一就绪内容立刻出现
        ...
        for (let i = 0; i < attempts; i++) {
          ...
          try {
            ...
            if (i < attempts - 1) {
              ...
              await sleep(350 + i * 280)
            }
          }
        }
        store.set({ error: lastErr, connecting: false })
        return false
      }
      ...
      function Panel() {
        ...
        return h('div', { className: 'tk-panel', style },
          ...
          store.perf ? h('div', { className: 'tk-perf', title: '数据通道与加载耗时' }, '加载 ' + store.perf.ms + 'ms · ' + (store.perf.transport === 'connection' ? '直连' : '命名空间')) : null,
