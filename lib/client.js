...
.tk-title .tk-logo{width:26px;height:26px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;font-size:14px;background:linear-gradient(135deg, var(--dsw-alias-brand-primary, #38bdf8), #7c6cf0);color:#fff}
...
.tk-skel{flex:1;min-height:120px;display:flex;flex-direction:column;gap:8px;justify-content:center}
.tk-skel-bar{height:14px;border-radius:7px;background:linear-gradient(90deg, var(--dsw-alias-bg-layer-2, rgba(255,255,255,.06)) 25%, var(--dsw-alias-bg-layer-1, rgba(255,255,255,.14)) 50%, var(--dsw-alias-bg-layer-2, rgba(255,255,255,.06)) 75%);background-size:200% 100%;animation:tkShimmer 1.4s infinite}
@keyframes tkShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
`
    const CSS_ID = '@cindy/taskman/ui'
    if (typeof document !== 'undefined' && document.querySelector('style[data-plugin-css=' + JSON.stringify(CSS_ID) + ']') === null) {
      const tag = document.createElement('style')
      tag.dataset.plugin = '@cindy/taskman'
      tag.dataset.pluginCss = CSS_ID
      tag.textContent = CSS
      document.head.appendChild(tag)
    }

    const inject = ['remote', 'slots', 'workspaces']
    ...
    function apply(ctx) {
      ...
      // 两者都带超时，绝不无限等待；失败自动重置、下次调用重试。
      const withTimeout = (p, ms, msg) => new Promise((res, rej) => {
        if (!timer || !ms) { p.then(res, rej); return }
        let done = false
        const t = timer.timeout(() => { if (!done) { done = true; rej(new Error(msg)) } }, ms)
        p.then((v) => { if (!done) { done = true; t(); res(v) } }, (e) => { if (!done) { done = true; t(); rej(e) } })
      })
      let connected = null
      const connect = () => {
        if (!connected) {
          connected = (async () => {
            ...
            if (conn && conn.rpc && typeof conn.rpc.call === 'function') {
              return { call: (path, ep, payload) => conn.rpc.call(path, ep, payload) }
            }
            // 2) remote.$mount 命名空间
            const rmt = ctx.get('remote')
            if (!rmt) throw new Error('Cindy 通信通道不可用（connection/remote 均缺失）')
            const passthrough = (typeSymbol) => ({ mode: 'strict', typeSymbol, schema: { parse: (v) => v } })
            await rmt.$mount({
            ...
            const ns = ctx.get('remote.cindy')
            if (!ns) throw new Error('Cindy 远程命名空间未挂载')
            return { call: (path, ep, payload) => ns.invoke(payload.args.method, payload.args.args).then((v) => ({ ok: true, value: v })) }
          })()
          connected.catch(() => { connected = null })
        }
        return connected
      }
      const call = async (m, a) => {
        const rpc = await withTimeout(connect(), 10000, 'Cindy 连接超时，请重试')
        const r = await withTimeout(rpc.call('/api', 'cindy/invoke', { args: { method: m, args: a || {} } }), 30000, 'Cindy 请求超时：' + m)
        if (!r || r.ok === false) {
      ...
      const refresh = async () => {
        try {
          ...
          store.set({ state: r.state, configured: r.configured, error: null })
        } catch (e) {
      ...
      function Panel() {
        ...
        // 窄屏夹紧：left 用 max() 保证不越界；宽高可记忆、可缩放
        const style = {
        ...
        return h('div', { className: 'tk-panel', style },
          h('div', { className: 'tk-head', onPointerDown: onPD, onPointerMove: onPM, onPointerUp: onPU, style: { cursor: 'move', userSelect: 'none' } },
            h('span', { className: 'tk-title' }, h('span', { className: 'tk-logo' }, '👩‍💼'), 'Cindy · 任务秘书'),
          ...
          h('div', { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' } },
            !st ? (store.error
              ? h('div', { className: 'tk-body', style: { alignItems: 'center', justifyContent: 'center' } },
                  ...
                  h('button', { className: 'tk-btn primary', onClick: () => refresh() }, '↻ 重试'))
              : h('div', { className: 'tk-body' }, h(Skeleton)))
          ...
          h('div', { className: 'tk-grip', title: '拖动调整大小', onPointerDown: onRS, onPointerMove: onRM, onPointerUp: onRU }, '⤡'))
      }
        ...
        return h('div', { className: 'tk-card', style: { padding: 12, cursor: 'default' } },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between', alignItems: 'flex-start' } },
            h('div', { style: { minWidth: 0 } },
              h('div', { className: 'tk-card-name' }, '👩‍💼 Cindy 任务秘书'),
      ...
      disposers.push(slots.inject('sidebar.footer.action', () => slots.register(
        ...
        () => h('button', { className: 'tk-footbtn', title: 'Cindy 任务秘书', onClick: () => store.set({ open: !store.open }) }, '👩‍💼', h('span', null, 'Cindy')),
      )))
      ...
      // 预热：后台建立通道并预取状态，首次打开即有数据（不阻塞 UI，超时自愈）
      connect().then(() => refresh()).catch(() => {})
      console.log('[cindy] Client 半边已启动（任务秘书）')
