...
`
    ...
    function apply(ctx) {
      ...
      const withTimeout = (p, ms, msg) => new Promise((res, rej) => {
        if (!ms) { p.then(res, rej); return }
        let done = false
        let clear = null
        if (timer && timer.timeout) {
          clear = timer.timeout(() => { if (!done) { done = true; rej(new Error(msg)) } }, ms)
        } else {
          const t = setTimeout(() => { if (!done) { done = true; rej(new Error(msg)) } }, ms)
      ...
      const connect = () => {
        if (!connected) {
          connected = (async () => {
            // 1) connection 直连（最稳、最快）；ctx.get 拿不到就用注入属性兜底
      ...
      const call = async (m, a) => {
        let rpc = null
        try {
          rpc = await withTimeout(connect(), 4000, 'Cindy 连接超时，请重试')
        } catch (e) {
          console.error('[cindy] 通道建立失败（transport=' + (lastTransport || '?') + '）', e)
          throw e
        }
        const r = await withTimeout(rpc.call('/api', 'cindy/invoke', { args: { method: m, args: a || {} } }), 8000, 'Cindy 请求超时：' + m)
        if (!r || r.ok === false) {
      ...
      const refresh = async (opts = {}) => {
        ...
        for (let i = 0; i < attempts; i++) {
          ...
          try {
            ...
            console.log('[cindy] get-state 成功', Date.now() - t0, 'ms ·', lastTransport === 'remote' ? 'remote' : 'connection', '· configured=' + store.configured)
            return true
          } catch (e) {
      ...
      function Panel() {
        ...
        return h('div', { className: 'tk-panel', style },
          ...
          store.perf ? h('div', { className: 'tk-perf', title: '数据通道与加载耗时 · build ' + BUILD }, '加载 ' + store.perf.ms + 'ms · ' + (store.perf.transport === 'connection' ? '直连' : '命名空间') + ' · ' + BUILD) : null,
