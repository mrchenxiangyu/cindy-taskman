...
.tk-meeting{border:1px solid var(--dsw-alias-border-l1, rgba(148,163,184,.2));border-radius:10px;padding:8px 10px;background:var(--dsw-alias-bg-layer-2, rgba(255,255,255,.04))}
.tk-report{max-height:74vh;overflow:auto;border-radius:12px;background:#fff}
...
`
    ...
    function apply(ctx) {
      ...
      // 写操作（建任务/产品/模板、更新、生成报告、会议等）耗时可能较长（建目录+注册工作区），
      // 超时放宽到 30s，避免「请求超时但服务端实际已成功」造成重名/重复操作；读操作保持 5s。
      const SLOW_CALLS = ['create-task', 'create-product', 'create-template', 'update-task', 'update-product', 'update-template', 'delete-task', 'delete-product', 'delete-template', 'add-log', 'generate-report', 'set-root', 'create-meeting', 'meeting-action', 'set-config', 'reconcile']
      const call = async (m, a, timeoutMs) => {
        ...
        const limit = timeoutMs || (SLOW_CALLS.indexOf(m) >= 0 ? 30000 : 5000)
        const tid = setTimeout(() => controller.abort(), limit)
      ...
      // 打开报告预览：优先 HTML 美化版，无则回退 Markdown
      const openReport = async (id, title) => {
        try {
          const htmlId = String(id).replace(/\.md$/, '.html')
          try {
            const r = await call('read-report', { id: htmlId })
            store.set({ reportView: { title: title || htmlId, content: r.content, html: true } })
            return
          } catch (e) { /* 无 HTML 版，回退 Markdown */ }
          const r = await call('read-report', { id })
          store.set({ reportView: { title: title || id, content: r.content, html: false } })
        } catch (e) { store.set({ error: String(e.message || e) }) }
      }
      function ReportModal() {
        useStore()
        const v = store.reportView
        if (!v) return null
        return h(Modal, { title: v.title, onClose: () => store.set({ reportView: null }), width: 880 },
          v.html
            ? h('div', { className: 'tk-report', dangerouslySetInnerHTML: { __html: v.content } })
            : h('pre', { className: 'tk-pre' }, v.content))
      }
      // refresh：失败立即显示错误（不干等重试），后台继续短间隔重试，成功即清错
      const refresh = async (opts = {}) => {
        // 先做目录对账：工作区里删除/改名了目录时，让 Cindy 记录与磁盘实况对应（缺失标记/路径跟随）
        try { await call('reconcile') } catch (e) { /* 对账失败不阻塞展示 */ }
      ...
      const STAGE_COLORS = ['#8b5cf6', '#3b82f6', '#f59e0b', '#10b981', '#6b7280']
      ...
      // 阶段随配置：config.stages（可在设置页编辑，最后一个阶段视为已关闭/归档），缺省五段
      const stagesOf = (st) => (st && st.config && Array.isArray(st.config.stages) && st.config.stages.length ? st.config.stages : DEFAULT_STAGES)
      const closedStageOf = (st) => { const s = stagesOf(st); return s[s.length - 1] }
      const stageMeta = (st, stage) => {
        const s = stagesOf(st)
        const i = s.indexOf(stage)
        if (i >= 0) {
          const closed = s[s.length - 1]
          if (stage === closed) return { color: '#6b7280', icon: '✅' }
          return { color: STAGE_COLORS[i % STAGE_COLORS.length] || '#94a3b8', icon: STAGE_ICONS[i] || '📍' }
        }
        const j = DEFAULT_STAGES.indexOf(stage)
        if (j >= 0) return { color: STAGE_COLORS[j % STAGE_COLORS.length], icon: STAGE_ICONS[j] || '📍' }
      ...
      function TaskCard({ t, state, sessionsByTask }) {
        ...
        const sm = stageMeta(state, t.stage)
        const sc = sm.color
        const pc = PRI_COLORS[t.priority] || '#94a3b8'
        const today = todayStr()
        const overdue = t.dueDate && t.dueDate < today && t.stage !== closedStageOf(state)
        const dueToday = t.dueDate === today && t.stage !== closedStageOf(state)
        ...
        return h('div', { className: 'tk-card', onClick: () => store.set({ detailTaskId: t.id }) },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between', flexWrap: 'nowrap' } },
            ...
            h('div', { className: 'tk-row', style: { gap: 4, flexWrap: 'nowrap' } },
              wsSvc ? h('button', { className: 'tk-btn ghost', title: '打开工作区文件夹', style: { padding: '1px 7px' }, onClick: (e) => { e.stopPropagation(); try { wsSvc.openPath(t.folder).catch(() => {}) } catch (err) {} } }, '📂') : null,
              h(Badge, { text: sm.icon + t.stage, color: sc }))),
          ...
          h('div', { className: 'tk-row', style: { gap: 6, marginTop: 6 } },
            ...
            t.dirMissing ? h('span', { className: 'tk-dim', style: { color: 'var(--dsw-alias-state-error-primary)', fontSize: 11, fontWeight: 600 } }, '⚠️ 目录缺失') : null),
      ...
      function BoardTab({ state, sessionsByTask }) {
        ...
        const today = todayStr()
        const open = state.tasks.filter((t) => t.stage !== closedStageOf(state))
        ...
        const gen = async (kind) => {
          ...
          try {
            ...
            await refresh()
            await openReport(r.id, (kind === 'daily' ? '日报 ' : '周报 ') + r.date)
        ...
        return h('div', { className: 'tk-body' },
          ...
          group === 'product'
            ...
            : h('div', { className: 'tk-cols' }, stagesOf(state).map((stg) => {
                const list = state.tasks.filter((t) => t.stage === stg)
                const sm = stageMeta(state, stg)
                return h('div', { className: 'tk-col', key: stg },
                  h('div', { className: 'tk-col-head' },
                    h('span', { className: 'tk-col-title', style: { color: sm.color } }, sm.icon + stg + '（' + list.length + '）')),
      ...
      function MeetingItem({ m }) {
        ...
        const setStatus = async (a, status) => {
          try {
            await call('meeting-action', { meeting: m.id, action: a.id, status })
            await refresh()
          } catch (e) { store.set({ error: String(e.message || e) }) }
        }
        return h('div', { className: 'tk-meeting', style: { marginTop: 8 } },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between', cursor: 'pointer' }, onClick: () => setOpen(!open) },
            h('div', { style: { minWidth: 0 } },
              h('div', { style: { fontWeight: 600, fontSize: 13 } }, '📅 ' + m.title),
              h('div', { className: 'tk-dim', style: { fontSize: 11, marginTop: 2 } }, m.date + (m.time ? ' ' + m.time : '') + ' · 参会：' + (m.attendees && m.attendees.length ? m.attendees.join('、') : '未记录'))),
            h('div', { className: 'tk-row', style: { gap: 6, flex: 'none' } },
              h(Badge, { text: done + '/' + total + ' 完成', color: allDone ? '#22c55e' : '#38bdf8' }),
              overdue ? h(Badge, { text: '⚠ 逾期 ' + overdue, color: '#ef4444' }) : null,
              h('span', { className: 'tk-dim' }, open ? '▾' : '▸'))),
          open ? h('div', { style: { marginTop: 8, padding: '8px 10px', background: 'var(--dsw-alias-bg-layer-2, rgba(255,255,255,.06))', borderRadius: 8 } },
            m.summary ? h('div', { className: 'tk-dim', style: { fontSize: 12, marginBottom: 4 } }, '💬 ' + m.summary) : null,
            m.conclusions ? h('div', { style: { fontSize: 12, marginBottom: 8 } }, '🎯 ' + m.conclusions) : null,
            h('div', { className: 'tk-dim', style: { fontSize: 11, marginBottom: 4 } }, '行动项（切换状态即完成勾选）'),
            m.actions.length === 0 ? h('div', { className: 'tk-dim', style: { fontSize: 12 } }, '（无）')
            : m.actions.map((a) => h('div', { key: a.id, className: 'tk-row', style: { gap: 8, marginTop: 4, alignItems: 'center' } },
                h('span', { style: { fontSize: 12, flex: 1 } }, a.text + (a.owner ? '（' + a.owner + '）' : '') + (a.due ? ' · 截止 ' + a.due : '')),
                h('select', { className: 'tk-input', style: { width: 88, padding: '2px 6px', fontSize: 11 }, value: a.status, onChange: (e) => setStatus(a, e.target.value) },
                  ['待办', '进行中', '已完成', '已取消'].map((s) => h('option', { key: s, value: s }, s))))))
          : null)
      }
      ...
      function MeetingForm() {
        ...
        const [acts, setActs] = React.useState([{ text: '', owner: '', due: '' }])
        const [busy, setBusy] = React.useState(false)
        const [err, setErr] = React.useState(null)
        ...
        const submit = async () => {
          setBusy(true); setErr(null)
          try {
            await call('create-meeting', {
              task: store.newMeetingForTask, title, date, time,
              attendees: attendees.split(/[,，;；]/).map((s) => s.trim()).filter(Boolean),
              summary, conclusions,
              actions: acts.filter((a) => a.text.trim()).map((a) => ({ text: a.text.trim(), owner: a.owner.trim(), due: a.due })),
            })
            store.set({ newMeetingForTask: null })
            await refresh()
          } catch (e) { setErr(String(e.message || e)) } finally { setBusy(false) }
        }
        return h(Modal, { title: '记录会议', onClose: () => store.set({ newMeetingForTask: null }), width: 660 },
          h('div', { className: 'tk-grid2' },
            h(Field, { label: '会议主题' }, h('input', { className: 'tk-input', placeholder: '如：需求评审会', value: title, onChange: (e) => setTitle(e.target.value), autoFocus: true })),
            h(Field, { label: '参会人（逗号分隔）' }, h('input', { className: 'tk-input', placeholder: '张三, 李四', value: attendees, onChange: (e) => setAttendees(e.target.value) })),
            h(Field, { label: '日期' }, h('input', { className: 'tk-input', type: 'date', value: date, onChange: (e) => setDate(e.target.value) })),
            h(Field, { label: '时间段' }, h('input', { className: 'tk-input', placeholder: '14:00-15:30', value: time, onChange: (e) => setTime(e.target.value) }))),
          h(Field, { label: '讨论内容' }, h('textarea', { className: 'tk-input', rows: 3, value: summary, onChange: (e) => setSummary(e.target.value) })),
          h(Field, { label: '结论 / 接下来的规划' }, h('textarea', { className: 'tk-input', rows: 2, value: conclusions, onChange: (e) => setConclusions(e.target.value) })),
          h('div', { className: 'tk-sec' }, '行动项（接下来的规划）'),
          acts.map((a, i) => h('div', { key: i, className: 'tk-row', style: { gap: 6, marginBottom: 6 } },
            h('input', { className: 'tk-input', style: { flex: 2 }, placeholder: '要做什么', value: a.text, onChange: (e) => setAct(i, 'text', e.target.value) }),
            h('input', { className: 'tk-input', style: { flex: 1 }, placeholder: '负责人', value: a.owner, onChange: (e) => setAct(i, 'owner', e.target.value) }),
            h('input', { className: 'tk-input', style: { flex: 1 }, type: 'date', value: a.due, onChange: (e) => setAct(i, 'due', e.target.value) }),
            h('button', { className: 'tk-btn ghost', style: { padding: '2px 8px' }, onClick: () => setActs(acts.filter((_, j) => j !== i)) }, '✕'))),
          h('div', { className: 'tk-row' },
            h('button', { className: 'tk-btn', onClick: () => setActs([...acts, { text: '', owner: '', due: '' }]) }, '＋ 行动项')),
          err ? h('div', { className: 'tk-err', style: { marginTop: 8 } }, err) : null,
          h('div', { className: 'tk-row', style: { marginTop: 10 } },
            h('button', { className: 'tk-btn primary', disabled: busy || !title.trim(), onClick: submit }, '保存会议'),
            h('button', { className: 'tk-btn ghost', onClick: () => store.set({ newMeetingForTask: null }) }, '取消')))
      }
      ...
      function TaskDetail({ state, sessionsByTask }) {
        ...
        const taskMeetings = (state.meetings || []).filter((m) => m.taskId === t.id)
        const save = async () => {
          setBusy(true); setErr(null)
          try {
        ...
        return h(Modal, { title: '任务详情', onClose: () => store.set({ detailTaskId: null }), width: 780 },
          ...
          h('div', { className: 'tk-grid2' },
            h(Field, { label: '阶段' }, h('select', { className: 'tk-input', value: stage, onChange: (e) => setStage(e.target.value) },
              (stagesOf(state).indexOf(stage) < 0 ? [stage] : []).concat(stagesOf(state)).map((st) => h('option', { key: st, value: st }, st + (stagesOf(state).indexOf(st) < 0 ? '（已移除）' : ''))))),
            h(Field, { label: '优先级' }, h('select', { className: 'tk-input', value: pri, onChange: (e) => setPri(e.target.value) },
              ['高', '中', '低'].map((x) => h('option', { key: x, value: x }, x)))),
            h(Field, { label: '开始日期' }, h('input', { className: 'tk-input', type: 'date', value: start, onChange: (e) => setStart(e.target.value) })),
            h(Field, { label: '截止日期' }, h('input', { className: 'tk-input', type: 'date', value: due, onChange: (e) => setDue(e.target.value) }))),
          h(Field, { label: '备注' }, h('textarea', { className: 'tk-input', rows: 2, value: note, onChange: (e) => setNote(e.target.value) })),
          err ? h('div', { className: 'tk-err', style: { marginBottom: 8 } }, err) : null,
          h('div', { className: 'tk-row', style: { alignItems: 'flex-end' } },
            h('label', { className: 'tk-field', style: { flex: 1, marginBottom: 0, minWidth: 160 } },
              h('span', { className: 'tk-field-label' }, '进度 ' + progress + '%'),
              h('input', { type: 'range', min: 0, max: 100, value: progress, onChange: (e) => setProgress(Number(e.target.value)) })),
          ...
          h('div', { className: 'tk-sec' }, '📅 会议记录（' + taskMeetings.length + '）'),
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between', marginBottom: 4 } },
            h('span', { className: 'tk-dim', style: { fontSize: 11 } }, '记录讨论结果与行动项，跟踪规划完成情况'),
            h('button', { className: 'tk-btn primary', style: { padding: '3px 10px' }, onClick: () => store.set({ newMeetingForTask: t.id }) }, '＋ 记录会议')),
          taskMeetings.length === 0 ? h('div', { className: 'tk-empty' }, '暂无会议记录')
          : taskMeetings.slice().sort((a, b) => (a.date < b.date ? 1 : -1)).map((m) => h(MeetingItem, { key: m.id, m })),
      ...
      function CalendarTab({ state }) {
        ...
        const chip = (t) => {
          const sc = stageMeta(state, t.stage).color
      ...
      function JournalTab({ state }) {
        ...
        const view = (rep) => openReport(rep.id, rep.id)
        const add = async () => {
      ...
      function SettingsTab({ state }) {
        ...
        const [stagesText, setStagesText] = React.useState(
          state.config && Array.isArray(state.config.stages) && state.config.stages.length
            ? state.config.stages.join('\n')
            : DEFAULT_STAGES.join('\n'))
        ...
        const saveTpl = async () => {
          if (!tplEdit) return
          if (!tplEdit.name.trim()) { setErr('模板名不能为空'); return }
          setBusy(true); setErr(null)
          try {
            // 文件夹结构可留空 = 空模板（只建任务文件夹，不建子目录）
        ...
        const saveStages = async () => {
          setBusy(true); setErr(null)
          try {
            const list = String(stagesText).split('\n').map((s) => s.trim()).filter(Boolean)
            if (list.length === 0) throw new Error('至少需要一个阶段')
            await call('set-config', { stages: list })
            await refresh()
          } catch (e) { setErr(String(e.message || e)) } finally { setBusy(false) }
        }
        ...
        return h('div', { className: 'tk-body' },
          ...
          secCard('任务阶段（每个阶段一行，按顺序排列）',
            h('div', { className: 'tk-row', style: { alignItems: 'flex-start' } },
              h('textarea', { className: 'tk-input', rows: 6, style: { flex: 1, fontFamily: 'inherit' }, value: stagesText, onChange: (e) => setStagesText(e.target.value) }),
              h('button', { className: 'tk-btn primary', disabled: busy, onClick: saveStages, style: { alignSelf: 'flex-end' } }, '保存')),
            h('div', { className: 'tk-hint' }, '最后一行视为「已关闭/归档」：任务进入该阶段即视为完成，不再计入进行中与逾期。若某阶段被删除，处于该阶段的任务会保留原阶段名继续显示，可手动改到新阶段。')),
          tplEdit ? h(Modal, { title: tplEdit.id ? '编辑模板' : '新建模板', onClose: () => setTplEdit(null), width: 620 },
            ...
            h(Field, { label: '文件夹结构（每行一个；用 | 追加说明；支持嵌套路径如 docs/meeting；留空 = 空模板，只建任务文件夹）' }, h('textarea', { className: 'tk-input', rows: 8, value: tplEdit.itemsText, onChange: (e) => setTplEdit({ ...tplEdit, itemsText: e.target.value }) })),
      ...
      function Panel() {
        ...
        return h('div', { className: 'tk-panel', style },
          ...
          store.newMeetingForTask ? h(MeetingForm) : null,
          store.reportView ? h(ReportModal) : null,
