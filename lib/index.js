...
export function apply(ctx) {
  ...
  // 批量建目录。DSH shell 在 Windows 上是 powershell.exe、Linux 是 bash，
  // 两者都以 `;` 分隔语句，逐目录 mkdir 兼容两种 shell；
  // 注意不能用 `mkdir a b c` 多参数形式（PowerShell 不支持，且可能静默失败不抛错）。
  // shell 仅为尽力而为：无论 shell 是否报错，剩余未建成的目录一律用标记文件兜底补齐，
  // 只有兜底也失败才返回 false（否则 create-task 会报「无法创建任务目录」）。
  async function ensureDirs(paths) {
    ...
    if (shellSvc !== undefined) {
      try {
        const cmds = missing.map((p) => 'mkdir "' + String(fs.processPath ? fs.processPath(p) : p).replace(/"/g, '') + '"')
        const spec = shellSvc.resolve({ command: cmds.join('; ') })
        await shellSvc.run(spec)
      } catch (e2) {
    ...
    for (const p of missing) {
      ...
      try {
        await wt(p + '/.taskman-keep', '')
        if (!(await exists(p))) ok = false
      } catch (e) { ok = false }
    }
    return ok
  }
  ...
  async function createTemplateInner(args) {
    ...
    // 文件夹结构允许为空 = 空模板（新建任务只建任务文件夹本身，不建子目录）
