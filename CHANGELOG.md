...
- **支持空模板**：新建/编辑模板时「文件夹结构」留空 = 空模板（新建任务只创建任务文件夹 + README，不建子目录）

### 修复

- **模板点保存没反应**：此前文件夹结构为空时 `saveTpl` 静默 return（无任何提示）；现允许空结构正常保存，模板名缺失时在弹窗内明确报错

### 从上一版本（0.3.1）升级

1. `git pull` 获取 0.3.2 源码
...
- **新建任务报「无法创建任务」**：当任务目录已存在（如你自己放好数据再建任务）时，`ensureDirs` 的批量 `mkdir` 在 PowerShell 下**静默失败**（不抛错也不建目录），旧代码该分支直接返回失败且不走兜底 → 建任务中止。现改为：shell 仅尽力而为，剩余未建成的模板子目录一律用标记文件兜底补齐，只有兜底也失败才报错
- 批量建目录命令改为 `;` 分隔逐目录 `mkdir`（兼容 PowerShell/bash；多参数 `mkdir a b c` 在 PowerShell 下不支持）

### 从上一版本（0.3.0）升级

1. `git pull` 获取 0.3.1 源码
2. 重新运行安装脚本：`powershell -ExecutionPolicy Bypass -File install-cindy.ps1`
3. 重启 DSH
