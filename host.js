// =====================================================================
// 任务管家（taskman）— Host 半边
// 数据布局：<根目录>/.taskman/{config,products,templates,tasks}.json
//          + journal.jsonl（追加式活动日志）+ reports/*.md（日报/周报）
// 每个任务 = 根目录/<产品>/<任务名>/ 下的模板化目录结构，
// 且同一时间注册为 harness 工作区（任务文件夹即工作区）。
// =====================================================================
return {
  inject: ['timer'],
  apply(ctx) {
    ...
    if (fs === undefined || wsReg === undefined) {
      console.error('[taskman] 缺少 fs / workspaceRegistry 服务，插件无法工作')
      return
    }
    // 沙箱策略：插件是无会话上下文的受信任代码，默认策略（read-only/workspace-write）
    // 会拒绝写入用户选定的总目录，因此显式解析 full-access 策略并在每次写入时传入；
    ...
    // 互斥队列：所有入口（RPC / 定时 / 事件）先取锁再改状态，内部函数后缀 Inner 假定已持锁
    const mutex = (fn) => {
    ...
    async function ensureDir(path) {
      ...
      try { await wt(path + '/.taskman-keep', ''); return true } catch (e) {}
      try {
        if (shellSvc !== undefined) {
          const spec = shellSvc.resolve({ command: 'mkdir "' + String(path).replace(/"/g, '') + '"' })
          await shellSvc.run(spec)
          if (await exists(path)) return true
        }
      } catch (e2) {
        console.error('[taskman] mkdir 失败：' + path, e2)
      }
      return false
    }
    ...
    async function saveTableInner(name, data) {
      if (!config) return
      await ensureDir(config.root + '/.taskman')
      await wt(config.root + '/.taskman/' + name + '.json', JSON.stringify(data, null, 2))
    }
    function appendJournalInner(entry) {
      const e = { t: nowIso(), l: dateStr(new Date()) }
    ...
    async function setRootInner(rootPath) {
      ...
      await appendJournalInner({ kind: 'system', taskId: null, source: 'system', text: '任务管家已启用，根目录：' + root })
    ...
    async function createTaskInner(args) {
      ...
      for (const f of (tpl.files || [])) {
        ...
        } catch (e) { console.error('[taskman] 初始文件写入失败：' + f.path, e) }
      }
      ...
      } catch (e) { console.error('[taskman] 注册工作区失败', e) }
      const task = {
    ...
    async function deleteTaskInner(args) {
      ...
      await saveTableInner('tasks', tasks)
      await appendJournalInner({ kind: 'system', taskId: null, source: 'system', text: '任务「' + task.name + '」已从管家移除（目录与工作区保留）' })
      return { ok: true }
    }
    ...
    function isoWeekOf(d) {
      ...
      return { year: x.getUTCFullYear(), week: Math.ceil((((x - ys) / 86400000) + 1) / 7) }
    }
    ...
    async function schedulerTickInner() {
      ...
      if (!reports.some((r) => r.kind === 'daily' && r.date === today)) {
        try { await generateReportInner('daily', today) } catch (e) { console.error('[taskman] 日报生成失败', e) }
      }
      ...
      if (dow >= wd) {
        ...
        if (!reports.some((r) => r.kind === 'weekly' && r.date === wkey)) {
          try { await generateReportInner('weekly', wkey) } catch (e) { console.error('[taskman] 周报生成失败', e) }
        }
      }
    }
    ...
    async function refreshSessionMapInner() {
      ...
      try {
        ...
        for (const task of tasks) {
          const wid = task.workspaceId || byPath[norm(task.folder)]
          if (!wid) continue
          if (task.workspaceId !== wid) { task.workspaceId = wid; dirty = true }
      ...
      } catch (e) { console.error('[taskman] 会话映射刷新失败', e) }
    }
    ...
    for (const name of Object.keys(methods)) {
      harness.handle(name, async (args) => {
        try {
          return await mutex(() => methods[name](args || {}))
        } catch (e) {
    ...
    function tool(name, description, schema, run) {
      try {
        const def = harness.defineTool({
          ...
          execute: async (args) => {
            try {
              const r = await mutex(() => run(args || {}))
              return typeof r === 'string' ? r : JSON.stringify(r, null, 2)
            } catch (e) {
              return '[任务管家] 失败：' + String((e && e.message) || e)
            }
          },
        })
        harness.registerTool(ctx, def)
      } catch (e) {
        console.error('[taskman] 工具注册失败：' + name, e)
      }
    }

    tool('taskman_overview', '查看任务管家全局概览：根目录、任务数量与阶段分布、今日到期、逾期任务、最近报告。',
      {},
      async () => {
        if (!config) return '任务管家尚未设置根目录。请在「任务管家」面板的“设置”页选择总目录，或用 taskman_set_root 工具设置。'
    ...
    tool('taskman_set_root', '设置任务管家的总目录（所有产品、任务与管理数据都存放在该目录下）。',
      { path: { type: 'string', description: '总目录的绝对路径', required: true } },
      async (args) => {
        await setRootInner(args.path)
        return '已启用任务管家，根目录：' + config.root + '。产品 ' + products.length + ' 个，任务 ' + tasks.length + ' 个，模板 ' + templates.length + ' 个。'
      })

    tool('taskman_create_task', '创建任务：在产品下按模板生成任务文件夹结构，并注册为 harness 工作区（任务文件夹即工作区）。',
      {
      ...
      async (args) => {
        ...
        if (!product) throw new Error('产品不存在：' + args.product + '（可用 taskman_tasks 查看，或先在面板创建产品）')
    ...
    tool('taskman_progress', '更新任务进度：按 ID 或名称匹配任务，追加一条进展记录和/或更新进度百分比。',
      {
    ...
    tool('taskman_create_product', '创建产品：在根目录下建立产品文件夹（产品名即文件夹名）。',
      {
        name: { type: 'string', description: '产品名，如 B300雷达', required: true },
        note: { type: 'string', description: '备注（可选）' },
      },
      async (args) => {
        if (!config) throw new Error('尚未设置根目录，请先调用 taskman_set_root')
        const r = await createProductInner({ name: args.name, note: args.note })
        return '产品「' + r.product.name + '」已创建（文件夹：' + r.product.folder + '）'
      })

    tool('taskman_create_template', '创建文件夹模板：新建任务时按该模板一键生成目录结构；每行一个文件夹路径，可用 | 追加说明，支持嵌套路径。',
      {
    ...
    tool('taskman_daily_summary', '生成本日工作总结（Markdown，保存到根目录 .taskman/reports/）。',
      {},
      async () => {
    ...
    tool('taskman_weekly_report', '生成本周周报（Markdown，保存到根目录 .taskman/reports/）。',
      {},
      async () => {
    ...
    ctx.timeout(() => { refreshSessionMap(); schedulerTick() }, 5000)
    ctx.on('tools/result', onToolResult)
    ctx.on('agent/status', onAgentStatus)
    console.log('[taskman] Host 半边已启动')
  },
}
