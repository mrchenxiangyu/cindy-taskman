// =====================================================================
// 任务管家（taskman）— Client 半边
// 入口：侧边栏底部「任务管家」按钮 → shell.overlay 浮动面板
// 面板页签：总览（按产品/阶段分组）/ 日历（日·周·月）/ 甘特图 / 日志 / 设置
// 另在 cordis_run 卡片内注册快捷卡片（tool.view.cordis）
// =====================================================================
return {
  apply(ctx) {
    ...
    const store = {
      ...
      set(p) { Object.assign(store, p); store.notify() },
    }
    const useStore = () => {
    ...
    const refresh = async () => {
      try {
        ...
        let sessions = []
        if (r.configured) {
          try { const t = await call('task-sessions'); if (t.list) sessions = t.list } catch (e) {}
        }
        store.set({ state: r.state, configured: r.configured, sessions, error: null })
      } catch (e) {
    ...
    // ---------- 样式 ----------
    styles.insert(`
.tk-panel{background:rgba(15,23,42,0.98);color:#e5e7eb;border:1px solid rgba(148,163,184,0.3);font-size:13px;box-sizing:border-box}
.tk-panel *{box-sizing:border-box}
.tk-head{display:flex;align-items:center;gap:8px;padding:10px 14px;border-bottom:1px solid rgba(148,163,184,0.25);background:rgba(255,255,255,0.03)}
.tk-title{font-weight:600;font-size:14px;white-space:nowrap}
.tk-dim{color:#9ca3af}
.tk-err{color:#f87171;font-size:12px}
.tk-tabs{display:flex;gap:2px;padding:6px 10px 0;border-bottom:1px solid rgba(148,163,184,0.25)}
.tk-tab{background:none;border:none;color:#9ca3af;padding:7px 14px;cursor:pointer;font-size:13px;border-radius:8px 8px 0 0}
.tk-tab.on{color:#38bdf8;background:rgba(56,189,248,0.08)}
.tk-body{flex:1;min-height:0;overflow:auto;padding:14px;display:flex;flex-direction:column;gap:12px}
.tk-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.tk-btn{background:rgba(255,255,255,0.08);color:#e5e7eb;border:1px solid rgba(148,163,184,0.3);border-radius:8px;padding:6px 12px;cursor:pointer;font-size:12px;font-family:inherit}
.tk-btn:hover{background:rgba(255,255,255,0.14)}
.tk-btn.primary{background:#38bdf8;border-color:#38bdf8;color:#082f49;font-weight:600}
.tk-btn.danger{background:rgba(239,68,68,0.15);border-color:rgba(239,68,68,0.5);color:#f87171}
.tk-btn.ghost{background:none;border-color:transparent}
.tk-btn:disabled{opacity:0.5;cursor:default}
.tk-input{background:rgba(255,255,255,0.06);border:1px solid rgba(148,163,184,0.3);border-radius:8px;color:#e5e7eb;padding:6px 10px;font-size:12px;outline:none;font-family:inherit}
.tk-input:focus{border-color:#38bdf8}
.tk-stat{background:rgba(255,255,255,0.05);border:1px solid rgba(148,163,184,0.25);border-radius:10px;padding:8px 14px;min-width:64px;text-align:center}
.tk-stat-v{font-size:18px;font-weight:700}
.tk-stat-l{font-size:11px;color:#9ca3af}
.tk-cols{display:flex;gap:10px;flex:1;min-height:0;align-items:stretch}
.tk-col{flex:1;min-width:200px;display:flex;flex-direction:column;gap:8px;background:rgba(255,255,255,0.04);border:1px solid rgba(148,163,184,0.25);border-radius:10px;padding:10px;overflow:auto;max-height:100%}
.tk-col-head{display:flex;align-items:center;justify-content:space-between}
.tk-col-title{font-weight:600;font-size:13px}
.tk-card{background:rgba(255,255,255,0.04);border:1px solid rgba(148,163,184,0.25);border-radius:10px;padding:10px;cursor:pointer}
.tk-card:hover{border-color:#38bdf8}
.tk-card-name{font-weight:600;font-size:13px}
.tk-badge{display:inline-block;font-size:10px;padding:2px 7px;border-radius:999px;border:1px solid}
.tk-prog{height:6px;background:rgba(255,255,255,0.1);border-radius:3px;overflow:hidden}
.tk-prog>div{height:100%;background:#38bdf8}
.tk-empty{color:#9ca3af;font-size:12px;padding:14px;text-align:center;border:1px dashed rgba(148,163,184,0.3);border-radius:10px}
.tk-modal{position:fixed;inset:0;background:rgba(0,0,0,0.5);display:flex;align-items:center;justify-content:center;z-index:600;pointer-events:auto}
.tk-modal-card{width:720px;max-width:calc(100vw - 40px);max-height:calc(100vh - 60px);overflow:auto;background:rgba(15,23,42,0.99);color:#e5e7eb;border:1px solid rgba(148,163,184,0.3);border-radius:12px;padding:16px;box-shadow:0 20px 60px rgba(0,0,0,0.6)}
.tk-modal-title{font-weight:700;font-size:15px}
.tk-field{display:flex;flex-direction:column;gap:4px;margin-bottom:10px}
.tk-field-label{font-size:11px;color:#9ca3af}
.tk-grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.tk-sec{font-weight:700;font-size:12px;color:#38bdf8;margin-top:6px}
.tk-logs{display:flex;flex-direction:column;gap:4px;max-height:220px;overflow:auto}
.tk-log{display:flex;gap:8px;font-size:12px;padding:5px 8px;background:rgba(255,255,255,0.03);border-radius:6px;align-items:baseline}
.tk-log-t{color:#9ca3af;font-size:11px;white-space:nowrap}
.tk-log-k{color:#38bdf8;font-size:10px;border:1px solid rgba(56,189,248,0.4);border-radius:4px;padding:0 5px;white-space:nowrap}
.tk-log-x{flex:1}
.tk-cal-head{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.tk-cal-wh{text-align:center;color:#9ca3af;font-size:11px;padding:2px}
.tk-cal{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.tk-cal-cell{min-height:84px;border:1px solid rgba(148,163,184,0.25);border-radius:8px;padding:4px;overflow:hidden;background:rgba(255,255,255,0.03);cursor:pointer}
.tk-cal-cell.other{opacity:0.45}
.tk-cal-cell.today{outline:2px solid #38bdf8}
.tk-cal-dayhead{font-size:11px;color:#9ca3af;margin-bottom:2px}
.tk-cal-week{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}
.tk-cal-title{font-weight:600;font-size:14px;min-width:150px;text-align:center}
.tk-evt{font-size:11px;padding:1px 5px;border-radius:4px;margin-bottom:2px;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;background:rgba(255,255,255,0.05);border-left:3px solid #94a3b8}
.tk-gantt{flex:1;min-height:240px;overflow:auto;border:1px solid rgba(148,163,184,0.25);border-radius:10px;background:rgba(255,255,255,0.02)}
.tk-gantt-inner{min-width:100%}
.tk-g-row{display:flex;align-items:center;border-bottom:1px solid rgba(148,163,184,0.12);min-height:30px}
.tk-g-label{position:sticky;left:0;width:220px;min-width:220px;background:rgba(15,23,42,0.98);z-index:2;padding:0 8px;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.tk-g-group{font-weight:700;color:#9ca3af;background:rgba(255,255,255,0.04)}
.tk-g-h{position:absolute;top:0;height:22px;font-size:9px;color:#9ca3af;border-left:1px solid rgba(148,163,184,0.2);padding-left:2px;overflow:hidden}
.tk-g-grid{position:absolute;inset:0}
.tk-g-grid i{position:absolute;top:0;bottom:0;border-left:1px solid rgba(255,255,255,0.05)}
.tk-g-bar{position:absolute;top:4px;height:20px;border-radius:4px;cursor:pointer;border:1px solid;display:flex;align-items:center;padding:0 4px;overflow:hidden}
.tk-g-bar-label{font-size:10px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.tk-g-today{position:absolute;top:0;bottom:0;width:2px;background:#f87171;z-index:1}
.tk-pre{white-space:pre-wrap;font-family:ui-monospace,Consolas,monospace;font-size:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(148,163,184,0.3);border-radius:8px;padding:12px}
.tk-quick{background:rgba(15,23,42,0.9);border:1px solid rgba(148,163,184,0.3);border-radius:10px;padding:12px;color:#e5e7eb;font-size:13px}
.tk-quick-title{font-weight:700;font-size:13px}
.tk-warnline{font-size:12px;color:#fbbf24;margin-top:6px}
.tk-hero{font-size:40px;text-align:center}
.tk-footbtn{background:none;border:none;color:#9ca3af;cursor:pointer;font-size:12px;display:flex;align-items:center;gap:6px;padding:4px 8px;border-radius:8px;font-family:inherit}
.tk-footbtn:hover{color:#38bdf8;background:rgba(56,189,248,0.08)}
.tk-detail-head{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px;gap:10px}
`)
    ...
    function Stat({ label, value, warn, bad }) {
      return h('div', { className: 'tk-stat' },
        h('div', { className: 'tk-stat-v', style: { color: bad ? '#f87171' : warn ? '#fbbf24' : undefined } }, String(value)),
    ...
    function Modal({ title, onClose, children, width }) {
      return h('div', { className: 'tk-modal', onClick: (e) => { if (e.target === e.currentTarget) onClose() } },
        h('div', { className: 'tk-modal-card', style: width ? { width } : undefined },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between', marginBottom: 10 } },
    ...
    function TaskCard({ t, state, sessionsByTask }) {
      ...
      return h('div', { className: 'tk-card', onClick: () => store.set({ detailTaskId: t.id }) },
        h('div', { className: 'tk-row', style: { justifyContent: 'space-between', flexWrap: 'nowrap' } },
          ...
          h(Badge, { text: t.stage, color: sc })),
        h('div', { className: 'tk-row', style: { gap: 6, marginTop: 4 } },
        ...
        h('div', { className: 'tk-row', style: { gap: 6, marginTop: 6 } },
          ...
          overdue ? h('span', { className: 'tk-dim', style: { color: '#f87171', fontSize: 11 } }, '已逾期') : null,
          dueToday ? h('span', { className: 'tk-dim', style: { color: '#fbbf24', fontSize: 11 } }, '今日到期') : null),
        h('div', { className: 'tk-prog', style: { marginTop: 8 } }, h('div', { style: { width: t.progress + '%' } })),
        h('div', { className: 'tk-dim', style: { fontSize: 10, marginTop: 2, textAlign: 'right' } }, t.progress + '%'))
    }
        ...
        group === 'product'
          ...
          : h('div', { className: 'tk-cols' }, (state.config.stages || []).map((stg) => {
              ...
              return h('div', { className: 'tk-col', key: stg },
                h('div', { className: 'tk-col-head' },
                  h('span', { className: 'tk-col-title', style: { color: STAGE_COLORS[stg] || '#94a3b8' } }, stg + '（' + list.length + '）')),
      ...
      return h(Modal, { title: '任务详情', onClose: () => store.set({ detailTaskId: null }), width: 780 },
        h('div', { className: 'tk-detail-head' },
          h('div', null,
            ...
            p ? h('div', { className: 'tk-dim', style: { fontSize: 12 } }, '📦 ' + p.name + ' · 📁 ' + t.folder) : null),
          h('div', { className: 'tk-row' },
        ...
        h('div', { className: 'tk-logs' },
          ...
          : logs.map((e) => h(LogRow, { key: e.t + e.kind + Math.random(), e }))))
    }
    ...
    function CalendarTab({ state }) {
      ...
      if (view === 'day') {
        ...
        return h('div', { className: 'tk-body' }, head,
          ...
          entries.length ? entries.map((e) => h(LogRow, { key: e.t + e.kind + Math.random(), e })) : h('div', { className: 'tk-empty' }, '无记录'))
      }
      if (view === 'week') {
        const mon = addD(cursor, -((cursor.getDay() + 6) % 7))
        ...
        return h('div', { className: 'tk-body' }, head,
          h('div', { className: 'tk-cal-week' }, days.map((d) => {
            ...
            return h('div', { className: 'tk-cal-cell' + (ds === today ? ' today' : ''), key: ds },
              h('div', { className: 'tk-cal-dayhead', style: { color: ds === today ? '#38bdf8' : undefined } }, WEEK[(d.getDay() + 6) % 7] + ' ' + pad(d.getDate())),
              list.map(chip))
          })))
      }
      ...
      return h('div', { className: 'tk-body' }, head,
        ...
        h('div', { className: 'tk-cal' }, cells.map((d) => {
          ...
          list.length > 4 ? h('div', { className: 'tk-dim', style: { fontSize: 10 } }, '+' + (list.length - 4) + ' 更多') : null)
        })))
    }
    ...
    function GanttTab({ state }) {
      ...
      const bar = (t) => {
        ...
        return h('div', {
          ...
          style: { left, width, background: sc + 'cc', borderColor: sc },
      ...
      return h('div', { className: 'tk-body' },
        ...
        h('div', { className: 'tk-gantt' },
          h('div', { className: 'tk-gantt-inner', style: { width: 220 + nCols * colW } },
            h('div', { className: 'tk-g-row', style: { position: 'sticky', top: 0, zIndex: 3, background: 'rgba(15,23,42,0.98)' } },
    ...
    function JournalTab({ state }) {
      ...
      return h('div', { className: 'tk-body' },
        ...
        h('div', { className: 'tk-logs', style: { maxHeight: 260 } },
          ...
          : filtered.map((e) => h(LogRow, { key: e.t + e.kind + Math.random(), e }))),
        h('div', { className: 'tk-sec' }, '报告（' + (state.reports || []).length + '）'),
        h('div', { className: 'tk-cols', style: { maxHeight: 220, overflow: 'auto' } },
    ...
    function SettingsTab({ state }) {
      ...
      return h('div', { className: 'tk-body' },
        h('div', { className: 'tk-sec' }, '根目录'),
        state.config ? h('div', { className: 'tk-row' }, h('span', { className: 'tk-dim', style: { flex: 1 } }, '📁 ' + state.config.root)) : null,
        h('div', { className: 'tk-row' },
        ...
        h('div', { className: 'tk-dim', style: { fontSize: 11 } }, '管理数据保存在该目录下 .taskman/ 中（可随目录迁移、可 git 追踪）；产品与任务文件夹直接建在该目录下。'),
        err ? h('div', { className: 'tk-err' }, err) : null,
        h('div', { className: 'tk-sec' }, '产品管理'),
        h('div', { className: 'tk-row' },
          h('input', { className: 'tk-input', style: { width: 220 }, placeholder: '产品名，如 B300雷达', value: pName, onChange: (e) => setPName(e.target.value) }),
        ...
        h('div', { className: 'tk-cols', style: { maxHeight: 200, overflow: 'auto' } }, state.products.map((p) => h('div', { className: 'tk-card', key: p.id },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between' } },
            h('span', { className: 'tk-card-name' }, '📦 ' + p.name),
            h('button', { className: 'tk-btn danger', disabled: busy, onClick: () => delProduct(p.id) }, '删除')),
          p.note ? h('div', { className: 'tk-dim', style: { fontSize: 11 } }, p.note) : null))),
        h('div', { className: 'tk-sec' }, '文件夹模板（新建任务时一键生成目录结构）'),
        h('div', { className: 'tk-cols', style: { maxHeight: 220, overflow: 'auto' } }, state.templates.map((t) => h('div', { className: 'tk-card', key: t.id },
          h('div', { className: 'tk-row', style: { justifyContent: 'space-between' } },
            h('span', { className: 'tk-card-name' }, '🗂 ' + t.name + (t.builtin ? '（内置）' : '')),
            h('div', { className: 'tk-row' },
              h('button', { className: 'tk-btn ghost', onClick: () => setTplEdit({ id: t.id, name: t.name, itemsText: itemsText(t), note: t.note || '' }) }, '编辑'),
              !t.builtin ? h('button', { className: 'tk-btn danger', disabled: busy, onClick: () => delTpl(t.id) }, '删除') : null)),
          h('div', { className: 'tk-dim', style: { fontSize: 11 } }, (t.items || []).map((it) => it.path).join(' / '))))),
        ...
        h('div', { className: 'tk-dim', style: { fontSize: 11 } }, '到达设定时间后每 5 分钟检查一次：自动生成当日总结；到周报日后自动生成本周周报。报告为 Markdown，保存在根目录 .taskman/reports/。任务会话中的文件操作与启动/结束会通过 harness 事件自动写入日志。'),
    ...
    function Onboard() {
      ...
      return h('div', { className: 'tk-body' },
        h('div', { className: 'tk-hero' }, '📅'),
        h('div', { className: 'tk-quick-title', style: { textAlign: 'center' } }, '欢迎使用任务管家'),
        h('div', { className: 'tk-dim', style: { textAlign: 'center' } }, '第一步：选择一个总目录。所有产品文件夹、任务文件夹与管理数据（.taskman/）都将存放于此。'),
        h('div', { className: 'tk-row' },
          h('input', { className: 'tk-input', style: { flex: 1 }, placeholder: '如 D:\\Work 或 /home/me/work', value: p, onChange: (e) => setP(e.target.value) }),
          h('button', { className: 'tk-btn', disabled: busy, onClick: pick }, '📂 浏览…'),
    ...
    function QuickCard() {
      ...
      React.useEffect(() => {
        refresh()
        let disp = null
        if (timer) disp = timer.interval(() => { refresh() }, 60000)
        return () => { if (disp) disp() }
      }, [])
      const st = store.state
      if (!st) return h('div', { className: 'tk-quick' }, store.error ? '⚠️ 任务管家出错：' + store.error : '📅 任务管家加载中…')
      if (!store.configured) return h('div', { className: 'tk-quick' },
        h('div', { className: 'tk-quick-title' }, '📅 任务管家未初始化'),
        h('div', { className: 'tk-dim', style: { fontSize: 12, marginTop: 4 } }, '选择一个总目录后，即可按产品/模板管理任务、关联 harness 工作区并自动生成日报周报。'),
        h('button', { className: 'tk-btn primary', style: { marginTop: 8 }, onClick: () => store.set({ open: true, tab: 'settings' }) }, '去设置'))
      const stats = st.stats || {}
      const today = todayStr()
      const due = st.tasks.filter((t) => t.dueDate === today && t.stage !== '已关闭')
      const over = st.tasks.filter((t) => t.dueDate && t.dueDate < today && t.stage !== '已关闭')
      return h('div', { className: 'tk-quick' },
        h('div', { className: 'tk-row', style: { justifyContent: 'space-between' } },
          h('div', { className: 'tk-quick-title' }, '📅 任务管家'),
          h('button', { className: 'tk-btn primary', onClick: () => store.set({ open: true }) }, '打开面板')),
        h('div', { className: 'tk-row', style: { gap: 10, marginTop: 6 } },
          h('span', { className: 'tk-dim', style: { fontSize: 12 } }, '📁 ' + st.config.root),
          h('span', { className: 'tk-dim', style: { fontSize: 12 } }, '任务 ' + stats.total + ' · 进行中 ' + stats.active)),
        due.length ? h('div', { className: 'tk-warnline' }, '⏰ 今日到期：' + due.map((t) => t.name).join('、')) : null,
        over.length ? h('div', { className: 'tk-warnline', style: { color: '#f87171' } }, '⚠️ 已逾期：' + over.map((t) => t.name).join('、')) : null)
    }
    ...
    function Panel() {
      ...
      const pos = store.pos
      const style = {
        position: 'fixed', zIndex: 400, pointerEvents: 'auto',
        width: 980, maxWidth: 'calc(100vw - 32px)', height: 660, maxHeight: 'calc(100vh - 32px)',
        display: 'flex', flexDirection: 'column', borderRadius: 12, overflow: 'hidden',
        boxShadow: '0 18px 60px rgba(0,0,0,0.55)',
        top: pos ? pos.y : 72,
        left: pos ? pos.x : 'calc(50% - 490px)',
      }
      const st = store.state
      const sessionsByTask = {}
      for (const row of (store.sessions || [])) sessionsByTask[row.taskId] = row.sessions || []
      const onPD = (e) => {
        // 按钮区域不启动拖拽/捕获，否则 setPointerCapture 会吞掉按钮的 click
      ...
      return h('div', { className: 'tk-panel', style },
        h('div', { className: 'tk-head', onPointerDown: onPD, onPointerMove: onPM, onPointerUp: onPU, style: { cursor: 'move', userSelect: 'none' } },
          h('span', { className: 'tk-title' }, '📅 任务管家'),
          ...
          h('button', { className: 'tk-btn ghost', title: '刷新', onClick: () => refresh() }, '↻'),
          h('button', { className: 'tk-btn ghost', onClick: () => store.set({ open: false }) }, '✕')),
        ...
        h('div', { style: { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' } },
          !st ? (store.error ? h('div', { className: 'tk-body' }, h('div', { className: 'tk-err' }, '⚠️ ' + store.error)) : h('div', { className: 'tk-body' }, h('div', { className: 'tk-dim' }, '加载中…')))
    ...
    function SettingsPluginCard() {
      ...
      return h('div', { className: 'tk-card', style: { padding: 12, cursor: 'default' } },
        h('div', { className: 'tk-row', style: { justifyContent: 'space-between', alignItems: 'flex-start' } },
          h('div', null,
            h('div', { className: 'tk-card-name' }, '📅 任务管家'),
    ...
    slots.inject('sidebar.footer.action', () => slots.register(
      { name: 'sidebar.footer.action', id: 'taskman', label: '任务管家' },
      () => h('button', { className: 'tk-footbtn', title: '任务管家', onClick: () => store.set({ open: !store.open }) }, '📅', h('span', null, '任务管家')),
    ))
    slots.inject('shell.overlay', () => slots.register(
      { name: 'shell.overlay', id: 'taskman-panel' },
      () => h(Panel),
    ))
    ...
    slots.inject('settings.plugin.item', () => slots.register(
      { name: 'settings.plugin.item', id: 'taskman', order: 1, label: '任务管家' },
      () => h(SettingsPluginCard),
    ))
    console.log('[taskman] Client 半边已启动')
  },
}
